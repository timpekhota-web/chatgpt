...
export default function AdminPage() {
  ...
  const uploadImageIfNeeded = async () => {
    if (!selectedFile) return form.imageUrl.trim();
    ...
    if (!uploadRes.ok || !uploadData.imageUrl) {
      throw new Error(uploadData?.error || 'Błąd uploadu zdjęcia.');
    }
  ...
  const createProduct = async (e: FormEvent) => {
    e.preventDefault();
    if (!canSubmit) {
      setStatus('Uzupełnij poprawnie pola: opis min. 10 znaków, cena > 0.');
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        imageUrl: uploadedImageUrl || fallbackImage
      };
      ...
      setStatus(fieldErrors || data?.error || 'Nie udało się dodać produktu.');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Nie udało się dodać produktu.');
    } finally {
      setBusy(false);
    }
  };
      ...
      <form onSubmit={createProduct} className="space-y-4 rounded-xl border border-fuchsia-500/40 bg-slate-950/90 p-4 shadow-[0_0_20px_rgba(217,70,239,0.2)]">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <label className="space-y-1">
            ...
            <input className="w-full rounded border border-fuchsia-500/40 bg-slate-900 p-2" value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} />
          </label>

          <label className="space-y-1">
            <span className="text-sm text-slate-300">Slug</span>
            <div className="flex gap-2">
              <input className="w-full rounded border border-fuchsia-500/40 bg-slate-900 p-2" value={form.slug} onChange={(e) => setForm((prev) => ({ ...prev, slug: e.target.value }))} />
              <button type="button" className="rounded border border-cyan-400/50 px-3 text-sm text-cyan-200" onClick={() => setForm((prev) => ({ ...prev, slug: slugify(prev.name) }))}>
                Auto
              </button>
            </div>
          </label>
          ...
          <label className="space-y-1 md:col-span-2">
            ...
            <textarea className="w-full rounded border border-fuchsia-500/40 bg-slate-900 p-2" rows={3} value={form.description} onChange={(e) => setForm((prev) => ({ ...prev, description: e.target.value }))} />
          </label>

          <label className="space-y-1">
            <span className="text-sm text-slate-300">Cena (PLN)</span>
            <input className="w-full rounded border border-fuchsia-500/40 bg-slate-900 p-2" type="number" min={1} value={form.pricePln} onChange={(e) => setForm((prev) => ({ ...prev, pricePln: Number(e.target.value) }))} />
          </label>

          <label className="space-y-1">
            <span className="text-sm text-slate-300">Stan magazynu</span>
            <input className="w-full rounded border border-fuchsia-500/40 bg-slate-900 p-2" type="number" min={0} value={form.stock} onChange={(e) => setForm((prev) => ({ ...prev, stock: Number(e.target.value) }))} />
          </label>

          <label className="space-y-1 md:col-span-2">
            <span className="text-sm text-slate-300">URL zdjęcia (opcjonalnie)</span>
            <input className="w-full rounded border border-fuchsia-500/40 bg-slate-900 p-2" placeholder="https://..." value={form.imageUrl} onChange={(e) => setForm((prev) => ({ ...prev, imageUrl: e.target.value }))} />
          </label>

          <label className="space-y-1 md:col-span-2">
            <span className="text-sm text-slate-300">lub wrzuć zdjęcie z komputera</span>
            <input
              className="w-full rounded border border-cyan-500/40 bg-slate-900 p-2 file:mr-3 file:rounded file:border-0 file:bg-cyan-500 file:px-3 file:py-1 file:text-black"
              type="file"
              accept="image/*"
        ...
        <div className="flex flex-wrap gap-2">
          {quickTemplates.map((template) => (
            <button key={template.slug} type="button" className="rounded border border-cyan-500/50 px-3 py-1 text-sm text-cyan-200" onClick={() => setForm(template)}>
              Użyj szablonu: {template.name}
            </button>
          ))}
        </div>

        <button disabled={!canSubmit || busy} className="rounded bg-fuchsia-600 px-4 py-2 text-white disabled:opacity-60">
          Dodaj produkt
        </button>
      </form>
        ...
        {products.map((product) => (
          <div key={product.id} className="flex items-center justify-between rounded-xl border border-fuchsia-500/30 bg-slate-950/90 p-3">
            <div>
              ...
              <p className="text-sm text-slate-300">{(product.price / 100).toFixed(2)} PLN • stock: {product.stock}</p>
            </div>
            <button onClick={() => deleteProduct(product.id)} disabled={busy} className="rounded border border-red-400 px-3 py-1 text-sm text-red-300 disabled:opacity-60">
              Usuń
            </button>
          </div>
        ))}
